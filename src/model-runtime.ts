 ...
 */
export async function resolveReviewModel(reference: string, modelRuntime?: ModelRuntime): Promise<ResolvedReviewModel> {
  const runtime = modelRuntime ?? (await ModelRuntime.create()); // 复用调用方传入的 Runtime，避免重复加载模型目录。
  ...
  if (!resolved) {
    ...
    throw new Error(`无法解析模型 "${reference}"：${detail}。请检查配置中的 review.model 和 Pi 模型认证`);
  }
  return {
